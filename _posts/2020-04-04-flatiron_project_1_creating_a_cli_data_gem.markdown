...
1) An API class to get the data from the website i chose: 
```
class API
...
2) An Object class to save the character objects in and build methods that I could incorporate into my CLI class:
```
class Characters
...
3) A CLI class to provide users access to that data: 
```
class CLI 
